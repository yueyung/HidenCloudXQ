...
def handle_cloudflare(page):
    ...
    4. HidenCloud 自定义 Security Verification 页面
    """
    ...
    if "Just a moment" in page.title():
        log("⚠️ 检测到 Cloudflare JS Challenge (Just a moment...)，等待自动通过...")
...
def wait_and_handle_verification(page, target_selector=None, timeout=60):
    ...
    while time.time() - start_time < timeout:
        # 先处理验证
        if "Security Verification" in page.title() or "Just a moment" in page.title():
...
def login(page):
    ...
    if COOKIE_VALUE:
        ...
        try:
            ...
            # ❗关键修复：验证页不等于 Cookie 失效
            if "Security Verification" in page_title or "Just a moment" in page_title:
                log("⏳ 遇到验证页面，Cookie 可能仍然有效，等待通过...")
            ...
            # 如果还在验证页（handle_cloudflare 超时），不要误判为登录成功
            if "Just a moment" in page_title:
                log("❌ Cloudflare 验证超时，无法继续")
                page.screenshot(path="login_stuck_cf.png")
                return False
    ...
    try:
        ...
        # 如果被验证页拦截，等待验证通过
        if "Security Verification" in current_title or "Just a moment" in current_title:
            log("⏳ 登录页被验证拦截，等待通过...")
