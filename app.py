...
def handle_cloudflare(page):
    ...
    3. Cloudflare JS Challenge ("Just a moment...")
    4. HidenCloud Security Verification 页面
    """
    # ---- 1. iframe 验证 ----
    ...
    if page.locator(turnstile_selector).count() > 0:
        log("⚠️ 检测到 Cloudflare Turnstile，等待通过...")
        start_time = time.time()
        while time.time() - start_time < 60:
    ...
    # ---- 3. "Just a moment..." JS Challenge ----
    title = page.title()
    if "Just a moment" in title:
        log("⚠️ 检测到 Cloudflare JS Challenge (Just a moment...)，等待通过...")
        page.screenshot(path="cf_js_challenge.png")
        start_time = time.time()
        while time.time() - start_time < 90:
            page.wait_for_timeout(3000)
            title = page.title()
            if "Just a moment" not in title:
                log("✅ Cloudflare JS Challenge 已通过！")
                page.wait_for_timeout(2000)
                return True
        log("❌ Cloudflare JS Challenge 超时（90秒），可能需要更换代理IP")
        page.screenshot(path="cf_js_challenge_timeout.png")
        return False

    # ---- 4. Security Verification 页面 ----
    if "Security Verification" in title:
        log("⚠️ 检测到 Security Verification 页面，等待通过...")
            ...
            try:
                for btn_text in ["Verify", "Submit", "Continue", "验证", "确认"]:
                    ...
                    if btn.is_visible(timeout=1000):
                        log(f"🖱️ 点击 '{btn_text}' 按钮...")
                        btn.click()
                        page.wait_for_timeout(5000)
                        break
            except Exception:
                pass
        log("❌ Security Verification 超时（90秒）")
...
def is_verification_page(page):
    """判断当前是否处于安全验证页面"""
    title = page.title()
    return "Just a moment" in title or "Security Verification" in title
...
def login(page):
    ...
    if COOKIE_VALUE:
        ...
        try:
            ...
            if is_verification_page(page):
                ...
                handle_cloudflare(page)
                page_title = page.title()
                log(f"📝 验证后Title: {page_title}")

            # 还在验证页 → 不能算登录成功
            if is_verification_page(page):
                log("❌ 验证页面无法通过，Cookie 登录中止")
                page.screenshot(path="cookie_login_stuck.png")
                # 如果有密码，继续尝试密码登录；否则直接失败
                if not EMAIL or not PASSWORD:
                    return False
            elif "auth/login" not in page.url:
            ...
            # 关键修复：不再静默吞掉异常，记录出来方便排查
            log(f"⚠️ Cookie 登录异常: {type(e).__name__}: {e}")
    ...
    try:
        ...
        # 等待验证通过 + 登录表单出现（关键修复：不再裸 page.fill）
        log("⏳ 等待登录表单加载...")
        if not wait_for_element(page, 'input[name="email"]', timeout=45):
            log("❌ 登录表单未出现（可能仍被验证页拦截或页面结构已变化）")
            page.screenshot(path="login_form_missing.png")
            return False
